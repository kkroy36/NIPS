...
plt.plot(range(2000),Y,label="without transfer",color='red',linewidth=3)
plt.plot(range(2000),Ynew,label="with transfer",color='green',linewidth=3)
plt.xlabel("iterations")
plt.ylabel("Bellman error(avg)")
plt.ylim(ymin=-0.05,ymax=0.35)#max(difference(Yhuber,YNN))+0.5)
plt.title("Blackjack")
plt.legend()
plt.show()  
