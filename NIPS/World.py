...
def write_rdn_files_for_regression_training_and_testing(facts,pos):
    '''write train_facts and train_pos
       and empty train_neg and same for test
    '''
    with open("train/train_facts.txt", "a") as facts_file:
        for fact in facts:
            facts_file.write(fact+'\n')
    with open("train/train_pos.txt", "a") as pos_file:
        for ex in pos:
            pos_file.write(ex+"\n")
    call_process('touch train/train_neg.txt')
    with open("test/test_facts.txt", "a") as facts_file:
        for fact in facts:
            facts_file.write(fact+'\n')
    with open("test/test_pos.txt", "a") as pos_file:
        for ex in pos:
            pos_file.write(ex+"\n")
...
def train_regression_model():
    '''trains regression model
       on training data
    '''
    call_process('java -jar BoostSRL-v1-0.jar -reg -l -train train -target value')

def get_projected_values():
    '''projects learned values
       to span of tree features
       using RFGB inference
    '''
...
def main():
    ...
    for trajectory in range(number_of_trajectories):
        ...
        while not goal_state(state):
            ...
            random_action,random_truck,random_box = action_specification[0],action_specification[1],action_specification[2]
